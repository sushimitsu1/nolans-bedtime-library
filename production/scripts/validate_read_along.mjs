...
requireMatch(/function readCurrentPage\(\{enableReadAlong=true\}=\{\}\)[\s\S]*readAlongEnabled=true;[\s\S]*readAlongStoryId=activeStory\.id;/, 'Read Page must enable read-along for the active story');
...
requireMatch(/stopNarrationButton['"]\)\.disabled=!speechSupported \|\| \(!narrationActive && !readAlongEnabled\)/, 'Stop must remain available while read-along is enabled');
