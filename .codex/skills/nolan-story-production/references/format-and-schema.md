...
Store batch visual evidence at `production/staging/<slug>/visual-review.json`. Each page record must contain `requiredSubjectBoxes` in final 1448 x 1086 coordinates and final checks for subject placement, safe-zone compliance, panel integration, consistent panel size, consistent badge placement, and unobstructed story action.
